...
public class GameJFrame_Easy extends JFrame implements MouseListener , KeyListener , ActionListener {
    int[][] data=new int[11][11];//0:无，1：有 ，-2，2：标记，3:4-10：数量  -1:显示所有雷,-3:点击的有雷区域
    int flag=0;
    int boom=10;//记录剩余雷数量
...
//        getContentPane().repaint();
        ...
        JLabel jLabelRecord = new JLabel();
        jLabelRecord.setBounds(240,20,80,40);
        jLabelRecord.setBorder(new BevelBorder(2));
        getContentPane().add(jLabelRecord);
...
//            getContentPane().repaint();
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if(flag==1){//第一次点击后布雷，防止第一次点踩到雷
            Algorithm.initBoom(data,x+1,y+1,9,9,boom);
        }
        //标记
        if(record==1){
            FlagBoom(x+1,y+1);
        ...
        //判断是否有雷
        if(!Judge(x,y)){
            Algorithm.algorithm(data,x+1,y+1,9,9);
        }
        ResetImg();
